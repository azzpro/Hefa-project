...
public class ShiroConfig {
	...
	public ShiroFilterFactoryBean shirFilter(SecurityManager securityManager) {
		...
		shiroFilterFactoryBean.setLoginUrl("/hefa/api/client/member/noLogin");
		// 设置无权限时跳转的 url;
		shiroFilterFactoryBean.setUnauthorizedUrl("/hefa/api/client/member/noPermission");
		// 设置拦截器
		Map<String, String> filterChainDefinitionMap = new LinkedHashMap<>();
		// 开放登陆接口
		filterChainDefinitionMap.put("/hefa/api/client/member/login", "anon");
		// 开放健康检查接口
		filterChainDefinitionMap.put("/actuator/health","anon");
		
		// 开放合法系统调用购物车的接口，使此接口走签名认证的拦截器(WebConfig中需配置拦截次url)
		filterChainDefinitionMap.put("/hefa/api/client/selection/addProductToShoppingCart","anon");
